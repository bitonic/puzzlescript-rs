...
impl<'a> ParseState<'a> {
    ...
    fn prelude_line(&mut self) -> Result<(), ErrorMsg> {
        if self.lex_token("title")? {
            ...
            self.ast.prelude.background_color =
                assert_found("background color", self.lex_color()?)?;
    ...
    fn validate_entity_definition(&self, name: &str) -> Result<(), ErrorMsg> {
        if self.ast.entities.objects.contains_key(name)
            || self.ast.entities.legend.contains_key(name)
        {
    ...
    fn rule_line(&mut self) -> Result<(), ErrorMsg> {
        ...
        let body = match mb_rhs {
            ...
            Some((rhs_matchers, rhs_sounds)) => {
                ...
                for (lhs_matcher, rhs_matcher) in
                    lhs_matchers.into_iter().zip(rhs_matchers.into_iter())
                {
