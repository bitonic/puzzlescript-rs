...
impl<'a> CompileState<'a> {
            ...
            for entity in collision_layer.iter() {
              ...
              collision_layers
                    ...
                    match &self.legend[entity] {
                        LegendBody::Aggregate(aggregate_objects)
                            if aggregate_objects.len() == 1 =>
                        {
    ...
    fn expand_lhs_cell_entity(
        ...
        match &self.legend[&match_entity.entity] {
            LegendBody::Aggregate(objects) => {
                expand_qualifier(rule_direction, match_entity.qualifier)
                    .into_iter()
                    .map(|qualifier| {
                ...
                .flat_map(|match_entity| {
                    if match_entity.qualifier == Some(ast::EntityQualifier::Random) {
                        vec![match &self.legend[&match_entity.entity] {
                            LegendBody::Aggregate(objects) => {
                                RHSEntity::Random(vec![objects.iter().cloned().collect()])
                            }
                            LegendBody::Property(objects) => RHSEntity::Random(
                                objects.iter().map(|object| vec![object.clone()]).collect(),
                            ),
                        }]
                    } else {
                        ...
                        match &self.legend[&match_entity.entity] {
                            LegendBody::Aggregate(objects) => {
                                ...
                                match resolve_qualifier(rule_direction, match_entity.qualifier) {
                                    ...
                                    None => {
                                        let with_lhs_info =
                                            |lhs_info: &LHSEntityInfo| match lhs_info {
                                        ...
                                        let mb_from_overall = || match overall_lhs_info
                                            .get_entity(&match_entity.entity)
                                        {
                                            Unique(lhs_info) => with_lhs_info(&lhs_info),
                                            Ambiguous => None,
                                        };
                                        let mb_from_qualifier = || match overall_lhs_info
                                                        ...
                                                        if derived_matchers.len() == 1 {
                                                            ...
                                                            for object in objects {
                                                                match objects_map.get(object) {
                                                                    None => return None,
                                                                    Some(match_object) => matchers
                                                                        .push(RHSEntity::Object(
                                                                            QualifiedObject {
                                                                                object:
                                                                                    match_object
                                                                                        .object
                          ...
                          match_entity.qualifier, match_entity.entity, overall_lhs_info
                            ...
                            LegendBody::Property(_) => {
                                ...
                                let with_lhs_info = |lhs_info: &LHSEntityInfo| {
                                    match lhs_info {
                                        LHSEntityInfo::Aggregate { .. } => {
                                            panic!("Got LHS aggregate for property")
                                        }
                                        LHSEntityInfo::Property {
                                            original_qualifier,
                                            derived_qualifier,
                                            binder,
                                        } => match resolve_qualifier(
                                            rule_direction,
                ...
                .map(|matcher| {
                    big_cross_product_lhs_info(
                        &matcher
                            .iter()
                            .map(|cell_matcher| match cell_matcher {
                                ast::CellMatcher::Ellipsis => {
                                    vec![(LHSInfo::new(), CellMatcher::Ellipsis)]
                                }
    ...
    fn process_levels(&mut self) -> Result<(), ErrorMsg> {
        for level in self.ast.levels.iter() {
            match level {
                ast::Level::Message(message) => {
                    self.game.levels.push(Level::Message(message.clone()))
                }
...
mod tests {
    ...
    fn elementary_block_faker() {
        let file = include_str!("../../puzzlescripts/elementary/block_faker.pzl");
        let ast = parser::parse(file).unwrap();
        compile(&ast).unwrap();
    }
    ...
    fn elementary_by_your_side() {
        let file = include_str!("../../puzzlescripts/elementary/by_your_side.pzl");
        let ast = parser::parse(file).unwrap();
        compile(&ast).unwrap();
    }
